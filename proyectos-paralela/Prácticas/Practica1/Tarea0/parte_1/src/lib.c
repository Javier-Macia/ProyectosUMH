...
void filtrado_media(unsigned char** dest, unsigned char** orig, const int filas, const int columnas, const int radio){
    ...
    for ( i = 0; i < filas; i++) 
        for ( j = 0; j < columnas; j++) 
            dest[i][j] = pixel_media(orig, filas, columnas, i, j, radio);
}
...
void filtrado_mediana(unsigned char** dest, unsigned char** orig, const int filas, const int columnas, const int radio){
    ...
    for ( i = 0; i < filas; i++) 
        for ( j = 0; j < columnas; j++) 
            dest[i][j] = pixel_mediana(orig, filas, columnas, i, j, radio);
}
...
void filtrado_sobel(unsigned char** dest, unsigned char** orig, const int filas, const int columnas, const int radio){
    ...
    for ( i = 0; i < filas; i++) 
        for ( j = 0; j < columnas; j++) 
            dest[i][j] = pixel_sobel(orig, filas, columnas, i, j, radio);
}
...
unsigned char pixel_sobel(unsigned char** matrix, const int matrix_width, const int matrix_height, const int pixel_row, const int pixel_col, const int square_size){
    ...
    if (pixel_row == 0 || pixel_row == matrix_height-1 || pixel_col == 0 || pixel_col == matrix_width-1){ // Es un borde o una esquina
        for (i = pixel_row-1; i <= pixel_row+1; i++){ // Recorremos los 3x3 pixeles alrededor del pixel
            ...
            for (j = pixel_col-1; j <= pixel_col+1; j++){ 
                if (j < 0) // Corrección de índices por la izquierda
                    ...
                    nueva_columna = j; 
                sumC += matrix[nueva_fila][nueva_columna] * C[indice];
                sumF += matrix[nueva_fila][nueva_columna] * F[indice];
                indice++;
            }
        ...
        for(i=pixel_row-1; i<=pixel_row+1; i++){
            for(j=pixel_col-1; j<=pixel_col+1; j++){
                sumC += matrix[i][j] * C[indice];
                sumF += matrix[i][j] * F[indice];
                indice++;
            }
        }
    }
