...
string muestraPasaporte( Pasaporte pas){
    ...
    for (int i = 0; i < NDIGITOS; i++){
        s_digitos += to_string(pas.digitos[i]);
    }
...
bool operator > (const Pasaporte& p1, const Pasaporte& p2){
    int contador = 0;
    for (int i = 0; i < NPREFIJO; i++){
        if (p1.prefijo[i] > p2.prefijo[i])
            return true;
        else if (p1.prefijo[i] == p2.prefijo[i])
            contador++;
        else
            return false;
    }
    for (int i = 0; i < NDIGITOS; i++){
        if (p1.digitos[i] > p2.digitos[i])
            return true;
        else if (p1.digitos[i] == p2.digitos[i])
            contador++;
        else
            return false;
    }
    for (int i = 0; i < NSUFIJO; i++){
        if (p1.sufijo[i] > p2.sufijo[i])
            return true;
        else if (p1.sufijo[i] == p2.sufijo[i])
            contador++;
        else
            return false;
    }
    if (contador==NPREFIJO + NDIGITOS + NSUFIJO) // Todos son iguales
        return false;
    return true;
}
...
bool operator == (const Pasaporte& p1, const Pasaporte& p2){
    for (int i = 0; i < NPREFIJO; i++){
        if (p1.prefijo[i] != p2.prefijo[i]) return false;
    }
    for (int i = 0; i < NDIGITOS; i++){
        if (p1.digitos[i] != p2.digitos[i]) return false;
    }
    for (int i = 0; i < NSUFIJO; i++){
        if (p1.sufijo[i] != p2.sufijo[i]) return false;
    }
    return true;
}
